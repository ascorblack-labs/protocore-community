"""Evidence a run observed, and the lifecycle that seals and judges it.

The models here describe durable runtime facts. They intentionally do not
infer those facts from generated text: adapters and trusted tools create
evidence records, a run seals what it observed into a candidate, and a
verdict is attached to that candidate — never derived from it.

The loop reads these types directly. Choosing which checks run, resolving
them to implementations and budgeting them is a decision above the loop,
and those contracts live with whoever makes it.
"""
...
def freeze_json(value: Any) -> Any:
    ...
    Public because a layer above holds the other half of the same data model
    and compares the two through the boundary. Two byte-identical copies of
    this projection would agree until one of them was edited, and the only
    symptom would be that objects stopped being equal across the boundary.
    """
...
def freeze_json_object(value: Mapping[str, Any]) -> Mapping[str, Any]:
...
def thaw_json_object(value: Mapping[str, Any]) -> dict[str, Any]:
    """Thaw a JSON *object*, leaving its element types polymorphic."""
    return {key: thaw_json(item) for key, item in value.items()}


def canonical_frozen_object(value: Mapping[str, Any], *, field_name: str) -> Mapping[str, Any]:
    ...
    return freeze_json_object(restored)


def require_nonempty_identifier(value: str, *, field_name: str) -> str:
...
class RequirementsManifest(BaseModel):
    ...
    def _freeze_requirements(cls, value: Mapping[str, Any]) -> Mapping[str, Any]:
        return canonical_frozen_object(value, field_name="requirements")
...
class EvidenceRecord(BaseModel):
    ...
    def _freeze_metadata(cls, value: Mapping[str, Any]) -> Mapping[str, Any]:
        return canonical_frozen_object(value, field_name="evidence metadata")
...
class EvidenceProducerBinding(BaseModel):
    ...
    def _require_identifier(cls, value: str, info: Any) -> str:
        return require_nonempty_identifier(value, field_name=info.field_name)


class ToolEvidenceContext(BaseModel):
    """What one tool invocation may say about the evidence it produces.

    The three cells here travelled as three loose fields on the invocation
    context, beside identifiers that describe the run rather than the evidence.
    They are one thing: an invocation either carries a trusted provenance for
    what it observes, or it carries none and its records are refused. Naming
    that as a single value keeps the invocation context to what every tool
    needs and leaves this to the tools that produce evidence.

    Absent (``None`` on the context) means the same as an empty one here: no
    origin and no binding, so any record the tool returns is rejected rather
    than admitted with an invented provenance.
    """

    model_config = ConfigDict(frozen=True)

    origin: RunTreeOrigin | None = None
    """Trusted immutable run-tree identity for evidence produced here.

    Root callers retain the historical minimal shape.  The engine always
    supplies the complete origin, including parent/subagent identifiers for
    descendants, so tools never infer provenance from text or metadata.
    """

    producer_binding: EvidenceProducerBinding | None = None
    """Registered producer binding for this execution.
    ...
    def with_producer_binding(
        self, binding: EvidenceProducerBinding | None
    ) -> ToolEvidenceContext:
        """A copy naming the producer the dispatcher resolved."""
        return self.model_copy(update={"producer_binding": binding})
...
class InheritedEvidencePrefix(BaseModel):
    ...
    def _require_unique_nonempty_record_ids(self) -> InheritedEvidencePrefix:
        ...
        for record_id in self.record_ids:
            require_nonempty_identifier(record_id, field_name="inherited evidence record_ids")
